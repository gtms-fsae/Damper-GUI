...
front_unsprung_mass=15.31 %lbs
rear_unsprung_mass=14.975 %lbs
...
%% Analyzing Frequency of Loading
    %Calculating Necessary Values
fr_shaft_acc=diff(dam_fr_speedins)/sample_time;
fr_tire_fz=(fr_shaft_acc/motion_ratio_front)*front_unsprung_mass*0.0115;
N=numel(fr_shaft_acc);
tmax=(N-1)*sample_time;
ts=1/sample_rate;
t=0:ts:tmax;
f=-sample_rate/2:sample_rate/(N-1):sample_rate/2;
fft_fr=fftshift(fft(fr_tire_fz));
figure('Name','FFT_FR','NumberTitle','off');
plot(abs(f),abs(fft_fr))
grid on
